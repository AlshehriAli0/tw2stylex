...
const reasons = (classes: string) => run(classes).resolved.refusals.map((r) => r.reason).sort();
...
describe('conditions become nested values with a default', () => {
  ...
  test('StyleX allows one nesting level, so selectors collapse into one key', () => {
    const bg = run('data-disabled:data-checked:bg-brand').ns.backgroundColor as Record<string, unknown>;
...
describe('selfSelector distinguishes self from relational', () => {
  ...
  test('escaped commas inside a value do not split the selector list', () => {
    expect(selfSelector('.ease-\\[cubic-bezier\\(0\\.22\\,1\\)\\]', 'ease-[cubic-bezier(0.22,1)]')).toBe('');
  });
});
...
describe('the verification gate catches real breakage', () => {
  ...
  test('condition erasure is caught', () => {
    ...
    const v = verifyNamespace('t', resolved, ns);
    expect(v.ok).toBe(false);
    expect(v.kind === 'mismatch' && v.mismatches.some((m) => m.condition.includes(':hover'))).toBe(true);
  });
  ...
  test('clean output verifies', () => {
    for (const c of ['flex items-center gap-2 p-4', 'text-sm hover:bg-accent md:flex', 'shadow-md ring-2'])
